//
//  DoYouSmokeViewController.swift
//  Glimpse
//
...
class DoYouSmokeViewController: UIViewController ,UITableViewDataSource,UITableViewDelegate, WebServiceDelegate {
    
    @IBOutlet weak var imageViewBG: UIImageView!
    @IBOutlet weak var tableViewSmoke: UITableView!
    
    
    var arraySmoke = NSArray()
    ...
    func getOptions()
    {
        ...
        dict.setObject("Do You Smoke?", forKey: USER.kQUESTION as NSCopying)
    ...
    func configView() {
        ...
        self.tableViewSmoke.tableFooterView = UIView.init()
        self.tableViewSmoke.dataSource = self
        self.tableViewSmoke.delegate = self
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "SmokeCell", for: indexPath) as! GlobalTableViewCell
        
        let religion = self.arraySmoke.object(at: indexPath.row) as! NSDictionary
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let body = self.arraySmoke.object(at: indexPath.row) as! NSDictionary
    ...
    func getDataFormWebService(jsonResult: NSDictionary, methodName: NSString) {
        ...
        if methodName.isEqual(to: APIMETHOD.kAPI_GET_OPTIONS_BY_QUESTION) {
            ...
            self.arraySmoke = (jsonResult.object(forKey: EXTRA.kAPI_OPTIONS as NSString) as? NSArray)?.mutableCopy() as! NSMutableArray
            
            tableViewSmoke.reloadData()
            
        }
        else if methodName.isEqual(to: APIMETHOD.kAPI_UPDATE_ADVANCE_OPTION) {
            
            let welcomeToGlimpseViewController = Alert.GET_VIEW_CONTROLLER(identifier: SB_ID.SBI_WELCOME_GLIMPSE_VC as NSString) as! WelcomeToGlimpseViewController
            self.MOVE(vc: welcomeToGlimpseViewController, animated: true)
        }
        //
